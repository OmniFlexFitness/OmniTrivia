...
import { GameState, GamePhase, GameMode, Player, Question, RoundConfig, Category } from '../types';
...
export const GameProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    let botInterval: any;
    if (state.isHost && state.phase === GamePhase.LOBBY && state.players.length < 3) {
  ...
  const initHost = () => {
    setState(prev => ({ ...prev, isHost: true, phase: GamePhase.HOST_CONFIG }));
  };
  ...
  const generateGame = async (rounds: number, questions: number) => {
    setState(prev => ({ ...prev, loading: true, totalRounds: rounds, questionsPerRound: questions }));
    ...
    try {
      ...
      if (selectedCats.length < rounds) {
        ...
        while(selectedCats.length < rounds) {
           selectedCats.push(CATEGORIES[Math.floor(Math.random() * CATEGORIES.length)]);
        }
      }
      ...
      for (let i = 0; i < rounds; i++) {
        ...
        const generatedQuestions = await generateQuestions(category.name, questions);
      ...
      setState(prev => ({ ...prev, loading: false, error: "Failed to generate game content." }));
    }
  };
